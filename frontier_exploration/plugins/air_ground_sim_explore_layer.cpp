...
#define uav_imu_height 0.2
#define ugv_laser_height 0.3
...
namespace frontier_exploration
{
    ...
    void AirGroundSimExploreLayer::onInitialize(){
        ...
        uav_mat_pub_ = nh_.advertise<frontier_exploration::MultiArrayWithHeader>("uav_mat",5);
        ugv_mat_pub_ = nh_.advertise<frontier_exploration::MultiArrayWithHeader>("ugv_mat",5);
        ...
        trigger_sub_ = nh_.subscribe( "/traj_start_trigger", 100, &AirGroundSimExploreLayer::traj_trigger_callback, this);
	...
	nh_.param<bool>("/flag_is_sim", flag_is_sim_, true);
        ...
        uav_a = Eigen::VectorXd::Zero(3);
        ugv_a = Eigen::VectorXd::Zero(3);
        ...
        for(int i=0; i<delay_time_count; i++) delay_time[i]=0.0;
        path_search_count=0;
        prev_plan_time=0;
                                                            ...
                                                            geometry_msgs::PoseStamped &next_frontier_air)
    {
        ...
        while(true) {
            ...
            if(path_search_count>0){
                double sum=0.0;
                if(path_search_count<5){
                    for(int i=0; i<path_search_count; i++){
                        sum +=delay_time[i];
                    }
                    delay_est=sum/path_search_count;
                }
                else{
                    for(int i=0; i<delay_time_count; i++){
                        sum +=delay_time[i];
                    }
                    delay_est=sum/delay_time_count;
                }

                Eigen::VectorXd uav_t_ = uav_coef.col(uav_coef.cols()-1);
                Eigen::VectorXd ugv_t_ = ugv_coef.col(ugv_coef.cols()-3);
                Eigen::VectorXd ugv_beta_ = ugv_coef.col(ugv_coef.cols()-4);
                Eigen::VectorXd uav_pos_est = Eigen::VectorXd::Zero(4);
                Eigen::VectorXd ugv_pos_est = Eigen::VectorXd::Zero(6);
                ugv_pos_est(4) = ugv_coef(0,ugv_coef.cols()-2); 
                ugv_pos_est(5) = ugv_coef(0,ugv_coef.cols()-1);

                std::cout<<"delay_est:"<<delay_est<<std::endl;

            double dT = uav_odom_.header.stamp.toSec()+delay_est*1.25;

            double dT_ugv = ugv_odom_.header.stamp.toSec()+delay_est*1.25;

            for (int i = 0; i < uav_t_.size(); i++) {
                        if (dT <= uav_t_(i)) {
                            double tt = i ? dT - uav_t_(i - 1) : dT - prev_plan_time;
                            Eigen::Matrix<double, 1, 6> t_p;
                            t_p << 1, tt, pow(tt, 2), pow(tt, 3), pow(tt, 4), pow(tt, 5);
                            Eigen::Matrix<double, 1, 6> t_v;
                            t_v << 0, 1, 2 * tt, 3 * pow(tt, 2), 4 * pow(tt, 3), 5 * pow(tt, 4);

                            Eigen::Matrix<double, 1, 6> t_a;
                            ...
                            uav_pos_est(0) = t_p * coef_x;
                            uav_pos_est(2) = t_p * coef_y;
                            uav_pos_est(1) = t_v * coef_x;
                            uav_pos_est(3) = t_v * coef_y;
                            break;
                        }
                        else if(i==uav_t_.size()-1){
                            dT=uav_t_(i);
                            i--;
                        }
                    }
                    ...
                    for(int i=0; i<ugv_t_.size();i++){
                        std::cout<<i<<std::endl;
                    if(dT_ugv<=ugv_t_(i)){
                        double tt =i>0? dT_ugv-ugv_t_(i-1) : dT_ugv-prev_plan_time;
                        ...
                        Eigen::VectorXd coef_x;
                        Eigen::VectorXd coef_y;
                        coef_x = (ugv_coef.block(i,0,1,6)).transpose();
                        coef_y = (ugv_coef.block(i,6,1,6)).transpose();
                        ugv_pos_est(0) = t_p * coef_x;
                        ugv_pos_est(2) = t_p * coef_y;
                        ugv_pos_est(1) = t_v * coef_x;
                        ugv_pos_est(3) = t_v * coef_y;

                        ugv_pos_est(4) += ugv_pos_est(5) * tt + 0.5 * ugv_beta_(i) * tt * tt;
                        ugv_pos_est(5) += ugv_beta_(i)*tt;
                        break;
                    }
                    else{
                        if(i==ugv_t_.size()-1){dT_ugv=ugv_t_(i); --i; continue;};
                        if(i>0){
                            double time_step=ugv_t_(i)-ugv_t_(i-1);
                            ugv_pos_est(4)+=ugv_pos_est(5) * time_step+0.5*ugv_beta_(i)*time_step*time_step;
                            ugv_pos_est(5)+=ugv_beta_(i)*time_step;
                        }
                    }
                }

                uav_pos=(uav_pos_est-uav_pos).norm()>0.5? uav_pos:uav_pos_est;
                ugv_pos=(ugv_pos_est-ugv_pos).norm()>0.3? ugv_pos:ugv_pos_est;
            ...
            path_search search(uav_pos, ugv_pos, this, &frontierA_cell_list, wx_min, wx_max, wy_min, wy_max, path_search_rho_, ground_range_,plan_time_start); // what is this???
            ros::Time time1=ros::Time::now();
            path_search_count++;
            delay_time[path_search_count%delay_time_count]=(time1-time0).toSec();
            prev_plan_time=search.get_plan_time_start();

            std::cout<<"Path_search time:"<<(time1-time0).toSec()<<"\n";
            ...
            if (flag_ugv_){
                ...
                if (search.find_max_path(search.get_ugv_tree(), ugv_coef, node_list, ugv_a)) { // if ugv tree has info gain
                ...
                std::cout<<"Delay time:"<<time2.toSec()-search.get_plan_time_start()<<"\n";
            ...
            if (flag_uav_)
            {
                ...
                if (search.find_max_path(search.get_uav_tree(), uav_coef, node_list, uav_a)) { // if uav tree has info gain
		    ...
		    uav_fail_flag = false;
                    ...
                    Time(0) = ((Path.row(0)-Path.row(1)).norm())/(uav_max_vx+uav_max_vy)*2;//(Vel.row(0).norm());//
		...
		if (is_send_uav_goal || fail_safe_cnt>1){
            //for(int i=0; i<30; i++)
                    uav_mat_pub_.publish(uav_mat);
                    ROS_ERROR("uav_mat_sent, time is %f", search.get_plan_time_start());
            ...
            */
            ...
            while (ros::Time::now().toSec() - loop_start_time < 2.0){
		ros::spinOnce();
		r.sleep();
	    }
    ...
    void AirGroundSimExploreLayer::uav_odom_call_back(const nav_msgs::Odometry &uav_odom_origin){
        ...
        uav_odom_.pose.pose.position.z+=uav_imu_height;
        //transformOdom(uav_odom_,"");

        //if(path_search_count>0) return;
        ...
        uav_a(0) = uav_odom_.twist.twist.angular.x;
        uav_a(1) = uav_odom_.twist.twist.angular.y;
    ...
    void AirGroundSimExploreLayer::ugv_odom_call_back(const nav_msgs::Odometry ugv_odom_origin){
        ...
        ugv_odom_.pose.pose.position.z+=ugv_laser_height;
    ...
    void AirGroundSimExploreLayer::traj_trigger_callback(const geometry_msgs::PoseStamped::ConstPtr& trigger_msg){
        std::cout<<"[#INFO] get traj trigger, start planning."<<std::endl;
        start_traj = true;
    }
}
